...
<caption>상담내역답변 수정</caption>
    ...
    <td>
		...
		<input name="emailAnswerAt" type="checkbox" disabled value="Y" <c:if test="${result.emailAnswerAt == 'Y'}">checked</c:if> title="이메일답변여부"> 이메일답변여부
  			...
  			<c:if test="${result.emailAnswerAt == 'Y'}">
  			    &nbsp;&nbsp;&nbsp;
				<a href = "<c:url value='/images/egovframework/com/cmm/search2.gif'/>" onClick="javascript:fn_egov_pop_mailform(); return false;" alt="발송메일 등록 팝업"></a>
 			</c:if>
  ...
  <tr>
    ...
    <td width="80%" nowrap>
      <textarea name="cnsltCn" class="readOnlyClass"  cols="300" rows="20"  style="width:450px;" title="상담내용" readonly><c:out value="${result.cnsltCn}"/>
      </textarea>
    </td>
  </tr>
